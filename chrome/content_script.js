/*
 * Tracks pushState, message listeners, and page changes.
 * Hijacks APIs and sends data to background via chrome.runtime.sendMessage.
 */
const injectedJS = function (
  pushstate,
  msgeventlistener,
  msgporteventlistener
) {
  const loaded = { value: false };
  const originalFunctionToString = Function.prototype.toString;

  const m = (detail) => {
    const storeEvent = new CustomEvent("postMessageTracker", { detail });
...
const injectedJSString = `(${injectedJS.toString()})(History.prototype.pushState, Window.prototype.addEventListener, MessagePort.prototype.addEventListener)`;
